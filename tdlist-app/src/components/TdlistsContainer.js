...
class TdlistsContainer extends Component {
  ...
  newTdlist = (e) => {
		if (e.key === 'Enter' && !(e.target.value === '')) {
		  axios.post("/api/v1/tdlists", { tdlist: { title: e.target.value } })
		  .then((res) => {
  ...
  modifyTdlist = (e, id) => {
		axios
      .put(`/api/v1/tdlists/${id}`, { tdlist: { done: e.target.checked } })
		  .then((res) => {
